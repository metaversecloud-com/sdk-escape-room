...
**Secrets:** no plaintext committed. `esc-room0-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret esc-room0-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `esc-room0-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `esc-room0` | `sdk-apps-dev` | esc-room0-dev-topia.topia-rtsdk.com | `/api/system/health` |
